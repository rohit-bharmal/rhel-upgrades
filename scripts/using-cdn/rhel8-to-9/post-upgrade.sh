...
# Source Version: RHEL 8
# Target Version: RHEL 9
...
# 1. System has been upgraded to RHEL 9
...
echo "This script will clean up after RHEL 8 to RHEL 9 upgrade."
...
echo "  • Successfully upgraded to RHEL 9 and rebooted"
echo ""
...
#==============================================================================
echo -e "\n${CYAN}Starting: Verifying the post-upgrade state of the RHEL 9 system${NC}\n"

# Step 2
step_echo 2 "Verify that the correct product is installed:"
...
# Step 3
step_echo 3 "Verify that the release version is set to the target OS version immediately after the upgrade:"
...
# Step 5
step_echo 5 "Locate remaining RHEL 8 packages:"
    # Locate remaining RHEL 8 packages:
    info_echo "Executing: rpm -qa | grep -e '\.el[78]' | grep -vE '^(gpg-pubkey|libmodulemd|katello-ca-consumer)' | sort"
    rpm -qa | grep -e '\.el[78]' | grep -vE '^(gpg-pubkey|libmodulemd|katello-ca-consumer)' | sort || { error_echo "Command failed: rpm -qa | grep -e '\.el[78]' | grep -vE '^(gpg-pubkey|libmodulemd|katello-ca-consumer)' | sort"; exit 1; }
...
step_echo 6 "[object Object]"
    ...
    info_echo "Executing: dnf -y remove leapp-deps-el9 leapp-repository-deps-el9"
    dnf -y remove leapp-deps-el9 leapp-repository-deps-el9 || { error_echo "Command failed: dnf -y remove leapp-deps-el9 leapp-repository-deps-el9"; exit 1; }



# Step 7
step_echo 7 "Optional: Remove all remaining upgrade-related data from the system:"
...
# Step 8
step_echo 8 "Disable DNF repositories whose packages are not RHEL 9-compatible. Repositories managed by RHSM are handled automatically. To disable these repositories:"
    ...
    info_echo "Executing: # Disable source version repository: rhel-8-for-x86_64-baseos-rpms
if subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-baseos-rpms" 2>/dev/null; then
    info_echo "Disabling repository: rhel-8-for-x86_64-baseos-rpms"
    dnf -y config-manager --set-disabled rhel-8-for-x86_64-baseos-rpms || warn_echo "Could not disable rhel-8-for-x86_64-baseos-rpms"
else
    info_echo "Repository rhel-8-for-x86_64-baseos-rpms is not enabled, skipping"
fi

# Disable source version repository: rhel-8-for-x86_64-appstream-rpms
if subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-appstream-rpms" 2>/dev/null; then
    info_echo "Disabling repository: rhel-8-for-x86_64-appstream-rpms"
    dnf -y config-manager --set-disabled rhel-8-for-x86_64-appstream-rpms || warn_echo "Could not disable rhel-8-for-x86_64-appstream-rpms"
else
    info_echo "Repository rhel-8-for-x86_64-appstream-rpms is not enabled, skipping"
fi

# Disable source version repository: codeready-builder-for-rhel-8-x86_64-rpms
if subscription-manager repos --list-enabled | grep -q "codeready-builder-for-rhel-8-x86_64-rpms" 2>/dev/null; then
    info_echo "Disabling repository: codeready-builder-for-rhel-8-x86_64-rpms"
    dnf -y config-manager --set-disabled codeready-builder-for-rhel-8-x86_64-rpms || warn_echo "Could not disable codeready-builder-for-rhel-8-x86_64-rpms"
else
    info_echo "Repository codeready-builder-for-rhel-8-x86_64-rpms is not enabled, skipping"
fi

# Disable source version repository: rhel-8-for-x86_64-supplementary-rpms
if subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-supplementary-rpms" 2>/dev/null; then
    info_echo "Disabling repository: rhel-8-for-x86_64-supplementary-rpms"
    dnf -y config-manager --set-disabled rhel-8-for-x86_64-supplementary-rpms || warn_echo "Could not disable rhel-8-for-x86_64-supplementary-rpms"
else
    info_echo "Repository rhel-8-for-x86_64-supplementary-rpms is not enabled, skipping"
fi"
    # Disable source version repository: rhel-8-for-x86_64-baseos-rpms
if subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-baseos-rpms" 2>/dev/null; then
    info_echo "Disabling repository: rhel-8-for-x86_64-baseos-rpms"
    dnf -y config-manager --set-disabled rhel-8-for-x86_64-baseos-rpms || warn_echo "Could not disable rhel-8-for-x86_64-baseos-rpms"
else
...
if subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-supplementary-rpms" 2>/dev/null; then
    ...
    info_echo "Repository rhel-8-for-x86_64-supplementary-rpms is not enabled, skipping"
fi || { error_echo "Command failed: # Disable source version repository: rhel-8-for-x86_64-baseos-rpms
if subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-baseos-rpms" 2>/dev/null; then
    info_echo "Disabling repository: rhel-8-for-x86_64-baseos-rpms"
    dnf -y config-manager --set-disabled rhel-8-for-x86_64-baseos-rpms || warn_echo "Could not disable rhel-8-for-x86_64-baseos-rpms"
else
...
# Step 9
step_echo 9 "Remove the existing rescue kernel and initial RAM disk:"
...
# Step 10
step_echo 10 "Reinstall the rescue kernel and related initial RAM disk:"
...
# Step 12
step_echo 12 "Verify that the previously removed rescue kernel and rescue initial RAM disk files have been created for the current kernel:"
...
# Step 13
step_echo 13 "Verify the rescue boot entry refers to the existing rescue files. See the grubby output:"
...
# Step 16
step_echo 16 "Verify SELinux mode"
    # Verify SELinux mode
