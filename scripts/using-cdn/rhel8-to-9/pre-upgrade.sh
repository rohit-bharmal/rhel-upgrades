...
echo "This script will prepare your system for RHEL 8 to RHEL 9 upgrade."
...
#==============================================================================
echo -e "\n${CYAN}Starting: Preparing a RHEL 8 system for the upgrade${NC}\n"
...
step_echo 3 "Ensure you have appropriate repositories enabled."
    ...
    # Check if repository rhel-8-for-x86_64-baseos-rpms is already enabled
    if ! subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-baseos-rpms" 2>/dev/null; then
      info_echo "Enabling repository: rhel-8-for-x86_64-baseos-rpms"
      subscription-manager repos --enable rhel-8-for-x86_64-baseos-rpms || { error_echo "Command failed: subscription-manager repos --enable rhel-8-for-x86_64-baseos-rpms"; exit 1; }
    else
      info_echo "Repository rhel-8-for-x86_64-baseos-rpms is already enabled, skipping"
    fi

    # Check if repository rhel-8-for-x86_64-appstream-rpms is already enabled
    if ! subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-appstream-rpms" 2>/dev/null; then
      info_echo "Enabling repository: rhel-8-for-x86_64-appstream-rpms"
      subscription-manager repos --enable rhel-8-for-x86_64-appstream-rpms || { error_echo "Command failed: subscription-manager repos --enable rhel-8-for-x86_64-appstream-rpms"; exit 1; }
    else
      info_echo "Repository rhel-8-for-x86_64-appstream-rpms is already enabled, skipping"
    fi
...
step_echo 4 "Note that you can also have the Optional or Supplementary repositories enabled:"
    ...
    # Check if repository codeready-builder-for-rhel-8-x86_64-rpms is already enabled
    if ! subscription-manager repos --list-enabled | grep -q "codeready-builder-for-rhel-8-x86_64-rpms" 2>/dev/null; then
      info_echo "Enabling repository: codeready-builder-for-rhel-8-x86_64-rpms"
      if subscription-manager repos --enable codeready-builder-for-rhel-8-x86_64-rpms; then
        info_echo "Successfully enabled optional repository: codeready-builder-for-rhel-8-x86_64-rpms"
      else
        warn_echo "Could not enable optional repository: codeready-builder-for-rhel-8-x86_64-rpms"
      fi
    else
      info_echo "Repository codeready-builder-for-rhel-8-x86_64-rpms is already enabled, skipping"
    fi

    # Check if repository rhel-8-for-x86_64-supplementary-rpms is already enabled
    if ! subscription-manager repos --list-enabled | grep -q "rhel-8-for-x86_64-supplementary-rpms" 2>/dev/null; then
      info_echo "Enabling repository: rhel-8-for-x86_64-supplementary-rpms"
      if subscription-manager repos --enable rhel-8-for-x86_64-supplementary-rpms; then
        info_echo "Successfully enabled optional repository: rhel-8-for-x86_64-supplementary-rpms"
      else
        warn_echo "Could not enable optional repository: rhel-8-for-x86_64-supplementary-rpms"
      fi
    else
      info_echo "Repository rhel-8-for-x86_64-supplementary-rpms is already enabled, skipping"
    fi



# Step 5
step_echo 5 "For systems subscribed using RHSM, lock the system to the desired source OS version:"
...
# Step 6
step_echo 6 "If you use the dnf versionlock plug-in to lock packages to a specific version, clear the lock by running:"
...
# Step 7
step_echo 7 "Install the Leapp utility:"
...
# Step 8
step_echo 8 "Update all packages to the latest RHEL 8 version:"
...
# Step 9
step_echo 9 "For reviewing through the web console, install cockpit-leapp package:"
...
# Step 10
step_echo 10 "Verify that the correct product is installed:"
